...
#paste motif ID and motif name
# removed to avoid pasting motif ids with NAs
#negative_set.df$motif.id <- paste(negative_set.df$motif_id
#                                  , negative_set.df$motif_alt_id, sep = "_")
#keep necessary columns: motif id, enhancer ID and cell type
...
#positive$motif.id <- paste(positive$motif_id, positive$motif_alt_id, sep = "_")
#keep necessary columns: motif id, enhancer ID and cell type
...
#move cell type column at the end
final_set <- final_set[,c(colnames(final_set)[colnames(final_set)!="celltype"],"celltype")]
#add a column to codify every cell type as binary (target =1, other =0)
